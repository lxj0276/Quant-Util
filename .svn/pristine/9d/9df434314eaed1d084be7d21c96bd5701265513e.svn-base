...
import redis
from redis_cache.env import DEBUG
from redis_cache.env import GLOBAL_CONNECTION
from redis_cache.utils import hash_args
...
def cache_it(limit=10000, expire=DEFAULT_EXPIRY, cache=None,
             use_json=False, namespace=None, connection=None):
    """
    ...
    def decorator(function):
        ...
        if cache is None:
            cache = SimpleCache(limit, expire, hashkeys=True, namespace=function.__module__, connection=connection)
        ...
        def func(*args, **kwargs):
            ...
            cache_key = '{func_source}:{key}'.format(func_source=function.__module__ + ':' + function.__name__,
                                                     key=key)
...
def cache_it_json(limit=10000, expire=DEFAULT_EXPIRY, cache=None, namespace=None):
    ...
    return cache_it(limit=limit, expire=expire, use_json=True,
                    cache=cache, namespace=None, connection=GLOBAL_CONNECTION.get_global_connection())


def cache_it_pickle(limit=10000, expire=DEFAULT_EXPIRY, cache=None, namespace=None, connection=None):
    return cache_it(limit=limit, expire=expire, use_json=False,
                    cache=cache, namespace=None, connection=GLOBAL_CONNECTION.get_global_connection())
...
def to_unicode(obj, encoding='utf-8'):
    ...
    #         obj = encode(obj, encoding)
    return obj


def cache_it_timeseries(limit=10000, expire=DEFAULT_EXPIRY, cache=None,
                        use_json=False, namespace=None, connection=None):
    """
    Arguments and function result must be pickleable.
    :param limit: maximum number of keys to maintain in the set
    ...
    def decorator(function):
        ...
        def func(*args, **kwargs):
            ...
            start_time = kwargs_copy.pop('start_time')
            end_time = kwargs_copy.pop('end_time')
            # -------------------------------------
            kwargs_copy.update({'is_time_key': True})
            time_key = hash_args(*args, **kwargs_copy)
            # -------------------------------------
            kwargs_copy.pop('is_time_key')
            value_key = hash_args(*args, **kwargs_copy)
            time_cache_key = '{func_source}:{key}'.format(func_source=function.__module__ + ':' + function.__name__,
                                                          key=time_key)
            if namespace:
                time_cache_key = '{namespace}:{key}'.format(namespace=namespace,
                                                            key=time_cache_key)
            try:
                start_cache_time, end_cache_time = fetcher(time_cache_key)
            except (ExpiredKeyException, CacheMissException) as e:
                result = function(*args, **kwargs)
                try:
                    storer(value_key, result, expire)
                    storer(time_cache_key, (start_time, end_time), expire)
                except redis.ConnectionError as e:
                    logging.exception(e)
            ...
            result = fetcher(value_key)
            if start_time >= start_cache_time and end_time <= end_cache_time:

                if time_frequance(result.index[0]) == FREQUANCE_MINUTE:  # 判断是否为分钟数据
                    return result[(result.index < end_time * 10000) & (result.index >= start_time * 10000)]
                else:
                    return result[(result.index < end_time) & (result.index >= start_time)]

            if start_time < start_cache_time:
                kwargs.update({'start_time': start_time, 'end_time': start_cache_time})
                left_append = function(*args,
                                       **kwargs)
                start_cache_time = start_time
            else:
                left_append = pd.DataFrame()

            if end_time > end_cache_time:
                kwargs.update({'start_time': end_cache_time, 'end_time': end_time})
                right_append = function(*args,
                                        **kwargs)
                end_cache_time = end_time
            else:
                right_append = pd.DataFrame()

            result = pd.concat([left_append, result, right_append], axis=0)
            try:
                storer(value_key, result, expire)
                storer(time_cache_key, (start_cache_time, end_cache_time), expire)
            except redis.ConnectionError as e:
                logging.exception(e)
            if time_frequance(result.index[0]) == FREQUANCE_MINUTE:  # 判断是否为分钟数据
                return result[(result.index < end_time * 10000) & (result.index >= start_time * 10000)]
...
if __name__ == '__main__':
    ...
    @cache_it_timeseries()
    def get_data3(x, start_time, end_time):
        print(x)
        date_range = pd.date_range(start_time, end_time)[:-1]
        columns = list('ABCD')
        np.random.shuffle(columns)
        df = pd.DataFrame(np.random.randn(date_range.__len__(), 4), columns=columns, index=date_range)
        return df


    df = get_data3(3, start_time='20171205', end_time='20171208')
